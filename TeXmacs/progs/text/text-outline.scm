...
(tm-define (document-outline)
  ...
  (with raw-sections (tree-search-sections (buffer-tree))
...
(tm-define (outline-go-to path-str)
  ;; 从路径字符串 "0:1:2" 解析并跳转，与 focus-section-menu 逻辑一致
  (with parts (map string->number (string-split path-str #\:))
    (when (and (list-every integer? parts) (not (null? parts)))
      (with t (path->tree parts)
        (when t (tree-go-to t 0 :end))))))
...
(tm-define (document-outline-widget)
  (resize "200px" "100%"
    (refreshable "document-outline-refresh"
      (vertical
        (for (item (document-outline))
          (horizontal
            ((eval (car item))
             ; 无路径的节点（如 the-index）不响应点击
             )))))))
